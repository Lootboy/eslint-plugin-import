...
ExportMap.for = function (context, options = {}) {
  ...
  if (!options.useCommonjsExports && !unambiguous.test(content)) {
...
ExportMap.parse = function (path, content, context, options = {}) {
  ...
  function captureDependency(declaration) {
    ...
    const getter = thunkFor(p, context, options)
    m.imports.set(p, {
      getter,
 ...
 */
function thunkFor(p, context, options) {
  return () => ExportMap.for(childContext(p, context), options)
}
